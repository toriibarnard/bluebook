# File: cbb_price_fetcher.py
# Canadian Blue Book API Price Fetcher for Excel Data - Python Version
...
class CanadianBlueBookPriceFetcher:
    ...
    def extract_wholesale_price(self, data: Dict) -> Optional[float]:
        """Extract wholesale price from API response."""
        # Look for common price fields
        price_fields = ['wholesalePrice', 'wholesale_price', 'value', 'price', 'tradeValue', 'trade_value']
        ...
        if isinstance(data, list) and data:
            return self.extract_wholesale_price(data[0])
        ...
        try:
            ...
            if vehicle['vin'] and len(str(vehicle['vin'])) > 10:
                ...
                if vin_result['success']:
                    result['cbb_wholesale_price'] = self.extract_wholesale_price(vin_result['data'])
                    result['lookup_method'] = vin_result['method']
                    result['api_response'] = json.dumps(vin_result['data'])
                    return result

            # Fallback to year/make/model search
            search_result = self.search_by_year_make_model(vehicle['year'], vehicle['make'], vehicle['model'])

            if search_result['success']:
                result['cbb_wholesale_price'] = self.extract_wholesale_price(search_result['data'])
